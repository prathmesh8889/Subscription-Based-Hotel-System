  ...
  return <div className="space-y-6 max-w-4xl">
    ...
    <div className="bg-white border rounded-xl p-5">
      ...
      <form onSubmit={changeCredentials} className="space-y-3">
        ...
        <button disabled={changing} className="bg-purple-600 text-white rounded-lg px-5 py-3 font-medium">{changing?'Updating...':'Change Login ID + Password Once'}</button>
      </form>:
      <div className="p-4 bg-gray-50 rounded-lg flex gap-3"><Lock className="text-gray-500"/><div><p className="font-medium">Credential change already used</p><p className="text-sm text-gray-500">This one-time security option is now locked{status.changedAt?' since '+new Date(status.changedAt).toLocaleString():''}.</p><p className="text-sm mt-2">Current login ID: <b>{status.currentEmail}</b></p></div></div>}
    </div>
