...
export function AdminAnalyticsPage(){
  ...
  return <div className="space-y-4 sm:space-y-6 min-w-0">
    ...
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">{cards.map(([label,value,Icon]:any)=><div key={label} className="bg-white border rounded-xl p-4"><Icon size={19} className="text-purple-600"/><p className="text-xs text-gray-500 mt-3">{label}</p><p className="text-xl font-bold">{value}</p></div>)}</div>
    <div className="grid lg:grid-cols-2 gap-5">
      <div className="bg-white border rounded-xl p-5"><h3 className="font-semibold mb-4">Plan Distribution</h3>{data.planDistribution.map((p:any)=><div key={p.plan} className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 py-3 border-b last:border-0"><span>{p.plan}</span><b>{p.count}</b></div>)}</div>
